...
app.get('/sunnyside', async (req, res) => {
        ...
        try {
            const client = await pool.connect();
            const result = await client.query(`SELECT *, ST_X(geom_sm), ST_Y(geom_sm) FROM centroids_wgs84_pluto_local WHERE ST_Intersects(ST_Buffer(ST_Transform(ST_SetSRID(ST_Point(${long}, ${lat}), 4326), 3857), 30), geom_sm);`);
            // const results = { 'results': (result) ? result.rows : null};
            // res.render('pages/db', results);
            res.send(result.rows);
            client.release();
